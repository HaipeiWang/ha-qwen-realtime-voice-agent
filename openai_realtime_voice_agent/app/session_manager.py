...
class RealtimeLLMUserAggregator(LLMUserAggregator):
    ...
    ms after VAD stop while Pipecat's default timeout is 500 ms.

    Keep transcript aggregation/context updates, but never manufacture another
    speech boundary from a late transcript in realtime mode.
    """

    async def _maybe_emulate_user_speaking(self):  # type: ignore[override]
        if (
            ...
            logger.info(
                "Late realtime transcript retained for context; duplicate emulated VAD suppressed"
            )


class RealtimeContextAggregatorPair(LLMContextAggregatorPair):
    """Context pair using the realtime-only no-emulated-VAD user aggregator."""

    def __init__(self, context: LLMContext):
        # The longer timeout associates ordinary 0.5-1.0 s final-transcript lag
        # with the real server-VAD turn.  The subclass above remains the hard
        # safety guarantee if a transcript arrives even later.
        user_params = LLMUserAggregatorParams(
            aggregation_timeout=1.5,
            turn_emulated_vad_timeout=1.5,
            enable_emulated_vad_interruptions=False,
        )
        self._user = RealtimeLLMUserAggregator(context, params=user_params)
        self._assistant = LLMAssistantAggregator(
            context, params=LLMAssistantAggregatorParams()
        )
...
class SessionManager:
    ...
    def create_context_aggregator(self, client_id: str) -> LLMContextAggregatorPair:
        ...
        aggregator_pair = RealtimeContextAggregatorPair(context)
