...
class AudioFrameRecorder(FrameProcessor):
    ...
    async def process_frame(self, frame: Frame, direction: FrameDirection):
        # FrameProcessor bookkeeping is required for every frame, not only the
        # StartFrame. Record before forwarding so a downstream cancellation or
        # transport failure cannot make the diagnostic WAV miss the packet.
...
class AudioRecordingService:
    ...
    def _initialize_recording(self):
        ...
        # Create the recorder and processor taps now, but open files only after
        # a real device connects. This avoids an empty startup pair and ensures
        # the active session owns the handles the pipeline processors use.
    ...
    def record_input_audio(self, audio_bytes: bytes):
        """Record PCM received at the transport boundary."""
        if self.enable_recording and self.audio_recorder:
            self.audio_recorder.record_input_audio(audio_bytes)

    def record_output_audio(self, audio_bytes: bytes):
        """Record PCM sent at the transport boundary."""
        if self.enable_recording and self.audio_recorder:
            self.audio_recorder.record_output_audio(audio_bytes)
    ...
    def start_new_session(self, client_id: Optional[str] = None):
        ...
        session_id = client_id or "session"
        self.audio_recorder = AudioRecorder(output_dir=self.output_dir)
